...
BinaryFileReader.prototype._readMatrix = function (ctx) {
    ...
    var type, // eslint-disable-line no-unused-vars
        length, // eslint-disable-line no-unused-vars
        flags,
        ndim,
    ...
    /* variable name */
    console.log(Buffer.from(ctx.data.buffer, ctx.offset, 32));
    type = this._readUInt32(ctx);
    length = this._readUInt32(ctx);
    name = Buffer.from(ctx.data.buffer, ctx.offset, length);
    ctx.offset += 8; /* skip over the name, fixed 8 bytes */
    ...
    this._printMatrix(dims, result);
};

/*
 * Prints out matrix in JSON format.
 */
BinaryFileReader.prototype._printMatrix = function (dims, data) {
    assert(dims.reduce((a, b) => { return a*b; }) === data.length);
    var result = [],
        i;
    function $() {
        var i,
            idx;
        assert(dims.length === arguments.length);
        for (i=0; i < dims.length-1; i++) {
            idx += arguments[i] * dims[i];
        }
        idx += arguments[dims.length-1];
        return data[idx];
    }
