...
BinaryFileReader.prototype._readMatrix = function (ctx) {
    var type,
        ...
        dims = [],
        off = ctx.offset;
    type = ctx.data.readUInt32LE(off),
    length = ctx.data.readUInt32LE(off+4),
    flags = ctx.data.readUInt32LE(off+8);
    ...
    ndim = length/4;
    off +=8;
    for (var i=0; i < ndim; i++, off+=4) {
        dims[i] = ctx.data.readInt32LE(off);
    }
    console.log("Dimensions:", ndim, dims);
    type = ctx.data.readUInt32LE(off);
    length = ctx.data.readUInt32LE(off+4);
    name = Buffer.from(ctx.data.buffer, off+8, length);
